...
- Coverage via pcov or xdebug with per-worker collection, incremental merge,
  five export formats, and a `coverage:diff` regression gate. The
  orchestrator process collects its own coverage on parallel runs, and CLI
  processes spawned during a covered run relay theirs back through a shared
  directory, so orchestrator-side code exercised by acceptance tests is
  measured too.
