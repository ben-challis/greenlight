...
generated.set('api.md', renderIndex(publicTypes));
...
function renderIndex(types) {
  ...
  for (const section of sections) {
    const count = types.filter((type) => sectionMatches(section, type)).length;
    lines.push(`- [${section.title}](${section.id}.md) — ${section.description} (${count} types)`);
  }
...
function sectionMatches(section, type) {
  return section.prefixes.some((prefix) => type.name.startsWith(prefix))
    || (section.names?.includes(type.name) ?? false);
}
