...
Greenlight is the testing framework PHP 8.4 deserves: fully typed, leak-free by design, parallel by default, and extensible through a plugin API that hands extensions the real runtime — not a sanitised event log.
...
Greenlight rejects both compromises. It is a new engine, not a skin: an attribute-driven authoring model that PHPStan and IDEs understand natively, an orchestrator/worker runner where parallelism and worker recycling are the core execution path, a native test-double library built on PHP 8.4 lazy objects with mandatory end-of-test teardown, and a plugin system whose interception points receive the actual test instance and its harness.

**One-line pitch:** *write typed tests, run them in parallel with flat memory, and extend the runner without fighting it.*
...
2. **Flat memory over arbitrary suite sizes.** A 50,000-test run must not consume meaningfully more peak memory per worker than a 500-test run. This is a design constraint, not an optimisation target.
3. **Parallel by default.** The out-of-box experience saturates available cores. Sequential execution is a configuration of the parallel runner (`workers: 1`), not a separate code path.
...
5. **First-class operational surface.** Output capture, multiple report formats, coverage export, and flow control (bail, retry, filtering, watch) are core capabilities with coherent APIs, not community add-ons.
6. **Ergonomics without magic.** Low ceremony comes from attributes, constructor injection, and a fluent expectation API — never from runtime code generation of test cases, closure rebinding, or global mutable state.
...
- **PHP < 8.4 support.** Lazy objects, property hooks, and asymmetric visibility are load-bearing in the design. Supporting older runtimes would force the same compromises we are escaping.
- **Browser / end-to-end testing.** Greenlight tests PHP code. Panther-style browser automation is plugin territory.
- **Mutation testing in core.** Infection-style mutation testing is a natural *plugin* (the plugin API is designed to make it possible), not a core capability.
...
Explicitly *not* the initial target: teams on legacy PHP versions, and teams who want drop-in PHPUnit compatibility. We trade addressable market at launch for coherence; PHP 8.4 adoption grows into the design, not the other way round.

## 5. Test authoring model

### 5.1 Shape of a test

Tests are `final` classes. Test methods, lifecycle hooks, and data sets are declared with attributes — never by method-name convention (`testFoo`, `setUp`) and never by closure DSL.
...
- **Classes, not closures.** A class instance gives Greenlight (and every plugin) a real object to hand around: typed properties, reflection-friendly metadata, deterministic construction and destruction. It is also the only model where "the plugin receives the actual test instance" is meaningful.
- **Attributes, not naming conventions.** `#[Test]` is explicit, greppable, and carries parameters (`#[Test(timeout: 5.0)]`). Magic method names are an implicit contract that tooling must special-case.
- **Constructor injection, not inheritance.** There is no mandatory `TestCase` base class. Tests declare what they need (expectation service, harness services) in the constructor; the runner provides them. No `parent::setUp()` footguns, no 300-method inherited API, and test classes remain instantiable in isolation.
- **Data sets are typed static methods** referenced by name via `#[DataSet]`, returning `iterable` with named keys. Enums implementing a `DataProvider` interface are also accepted. No string-encoded provider references to other classes without types.

### 5.2 Expectations

Assertions are a fluent, terminal expectation API delivered as an injected service (`Expect`), not a base-class method bag and not a global function (a `expect()` function wrapper is available for those who want it, but it resolves to the same service — no hidden state).
...
- Failures are **collected with rich diffs** (typed value renderers, not `var_export` dumps).
- **Soft expectations** are explicit: `$this->expect->softly(function (Expect $e) { ... })` gathers multiple failures before failing the test. Default remains fail-fast per expectation, because implicit soft-assertion modes hide broken tests.
- The vocabulary is extensible via **expectation plugins** (see §8), and extensions are *typed*: a plugin ships an interface extension + implementation, so `->toBeValidUuid()` autocompletes and analyses like core matchers.
...
Opinionated stance: **no `#[Depends]`.** Inter-test dependencies create hidden ordering contracts that break under parallelism. The supported pattern for expensive shared state is a suite- or class-scoped harness service (§9), which is explicit, typed, and parallel-safe.

## 6. Configuration

One mechanism: a `greenlight.php` file at the project root returning a fluent, fully typed builder — the PHPStan/Rector configuration model, chosen because it is IDE-discoverable, refactorable, and can *compute* (env-dependent worker counts, conditional plugins) without a templating layer.
...
- **No XML/YAML/JSON config, ever** (non-goal §3). One format means one documentation surface and no schema drift.
...
1. **Discovery** — the orchestrator statically discovers test classes (composer classmap + attribute scan; no test code is executed during discovery) and builds an execution plan.
2. **Distribution** — tests are distributed to workers deterministically (stable hash of class name) so a given seed + code state always yields the same placement. An optional timing cache (`.greenlight/timings`) rebalances by recorded duration to minimise tail latency.
3. **Execution** — each worker runs tests sequentially within itself: constructs the test class, resolves harness scopes, runs hooks and the test, tears down, reports structured results (events + captured output + timings + memory delta) back to the orchestrator as they happen — streaming, not end-of-run.
...
Rejected alternatives: *sequential-first with parallel later* (retrofitting parallelism is how PHPUnit ended up with paratest's limitations) and *Fiber-based single-process concurrency* (no crash isolation, no memory isolation, and PHP's ecosystem of blocking IO makes it a poor default; a Fiber-pool worker type may come later for IO-bound suites — see roadmap).

### 7.2 Flow control

- `--bail[=N]` stops the run after N failures (orchestrator drains workers gracefully).
...
- **Ordering:** default is defined order per class, randomised class order with printed seed. Randomisation surfaces hidden coupling early; per-method randomisation is opt-in.
...
The headline differentiator. PHPUnit's event system deliberately emits immutable, context-stripped value objects — a subscriber cannot see the test instance, cannot touch the container, cannot alter outcomes. That protects PHPUnit's internals at the cost of making whole categories of tooling impossible. Greenlight takes the opposite bet: **a smaller, versioned set of interception points that receive live context.**

- **Typed subscriber interfaces, not string event names.** A plugin implements e.g. `TestLifecycleSubscriber` with `beforeTest(TestContext $ctx)` / `afterTest(TestContext $ctx, TestResult $result)`. Registration is discovered from the interfaces the plugin class implements.
- **`TestContext` carries the live runtime:** the actual test instance, its `TestMetadata` (class, method, attributes, data-set key), the harness scope container, the run configuration, and an output sink. Plugins in `afterTest` receive the result *before* it is final and may annotate it or (via an explicit, logged API) transform it — this is how flaky-quarantine and retry policies get built outside core.
- **Capability-scoped plugin types**, each a separate interface: `Reporter` (consume the structured result stream), `ExpectationExtension` (add matchers), `HarnessProvider` (contribute injectable scoped services — this is how Symfony/Laravel bridges work), `RunLifecycleSubscriber` (orchestrator-side: run start/end, worker spawn/recycle), `TestLifecycleSubscriber` (worker-side). One plugin class may implement several.
- **Explicit execution side.** Every interception point is documented as orchestrator-side or worker-side, because they are different processes. The framework serialises what crosses the boundary; live objects (test instance, harness) are only available worker-side. This constraint is stated in the API rather than discovered in production.
- **Versioned plugin API.** The plugin surface has its own semver contract, narrower than the framework's internals, so internals can evolve without breaking the ecosystem — the lesson from PHPUnit's alternative (locking internals) and from its opposite (Pest plugins reaching into unversioned guts).

Trade-off, stated plainly: giving plugins live context means a misbehaving plugin can corrupt a test run. We accept this — the mitigations are worker isolation (blast radius is one worker), the logged outcome-transformation API (mutations are attributable in the report), and leak-checking plugins in CI (§12) rather than a crippled API.
...
- Expensive shared fixtures (a booted Symfony kernel, a migrated database, a Testcontainer) are `perClass`/`perSuite` harness services — the sanctioned replacement for `#[Depends]` and `setUpBeforeClass()` statics.
- PHP 8.4 **lazy objects** are used so scoped services are cheap to declare: a `perSuite` database harness costs nothing in suites that never touch it.
...
Every reporter renders from the same structured result model (status, timings, memory delta, captured output, expectation diffs, retry history), so no format is second-class. Custom reporters are ~50 lines of plugin.

## 11. Coverage

- **pcov is the recommended driver** (documented as such: it is dramatically faster); Xdebug is fully supported for branch/path coverage, which pcov cannot do.
- Collection is **per-worker**, merged incrementally by the orchestrator — no giant end-of-run merge spike, keeping goal 2 intact under coverage.
...
- **Baseline export/diff**: `greenlight coverage:diff --against=baseline.json` reports coverage change for the patch — designed for "coverage must not decrease" CI gates without third-party services.
...
5. **Worker recycling is the backstop, not the excuse.** Recycling exists because *user code* leaks; the framework itself must pass a CI job that runs 10,000 synthetic tests in one worker and asserts flat memory (< 1 MB drift).
...
Sequencing rationale: the runner comes first because every other component's API is shaped by the process boundary; doubles before coverage because doubles block real-world dogfooding; the plugin API stabilises last so two phases of internal use harden it before the semver promise.

## 16. Success metrics

- **Performance:** ≥ 2× wall-clock improvement vs PHPUnit + paratest on a public reference suite at equal worker counts; worker peak memory flat (< 1 MB drift) across a 10,000-test synthetic run.
- **Analysability:** a Greenlight test suite passes PHPStan level max with zero framework-specific extensions installed.
- **Extensibility proof:** flaky-test quarantine and a mutation-testing prototype implementable as external plugins without patching core (this is the acceptance test for §8's design).
- **Adoption proxies (12 months post-v1):** 3+ non-trivial open-source projects using it in CI; community plugins exist for at least one DI framework bridge not written by us.

## 17. Risks

| Risk | Assessment | Mitigation |
|---|---|---|
| PHP 8.4 floor limits early adoption | Real, accepted deliberately | Target greenfield projects and libraries first; adoption curve of 8.4 does the rest. Do not soften the floor. |
| Native doubles scope balloons | Highest-effort component | Contained scope (no final-class doubling in v1); ship in Phase 2, not 1; Mockery adapter as pressure valve. |
| Live-context plugin API enables misbehaving plugins | Accepted trade-off (§8) | Worker blast-radius containment, logged outcome mutations, API semver reviewed before GA. |
| Worker protocol complexity (serialisation across the process boundary) | Main technical risk in Phase 1 | Dedicated RFC before implementation; keep the wire model minimal (results, not objects); property-based tests on the protocol. |
| "Yet another framework" fatigue | Positioning risk | Lead with the two provable differentiators — flat memory at scale and the plugin API — with published benchmarks, not vibes. |
