...
final class ExecutionPlanTest
{
    ...
    public function exposesEntriesClassesAndCounts(): void
    {
        ...
        Expect::value($plan)->because('exposes entries classes and counts')->toHaveCount(3);
        Expect::value($plan->classes())->because('exposes entries classes and counts')->toBe(['App\FooTest', 'App\BarTest']);
        Expect::value($plan->seed)->because('exposes entries classes and counts')->toBe(7);

        $byClass = $plan->entriesByClass();

        Expect::value($byClass['App\FooTest'])->because('exposes entries classes and counts')->toHaveCount(2);
        Expect::value($byClass['App\BarTest'])->because('exposes entries classes and counts')->toHaveCount(1);
    }
    ...
    public function survivesTheWire(): void
    {
        ...
        Expect::value(\json_encode($restored->toWire(), \JSON_THROW_ON_ERROR))->because('survives the wire')
            ->toBe(\json_encode($plan->toWire(), \JSON_THROW_ON_ERROR));
        Expect::value($restored->seed)->because('survives the wire')->toBe(42);
        Expect::value($restored->entries[1]->id->dataSetKey)->because('survives the wire')->toBe('first case');
    }
