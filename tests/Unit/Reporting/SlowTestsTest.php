...
final class SlowTestsTest
{
    ...
    public function fastRunsRenderNothing(): void
    {
        ...
        expect($slow->render(new Style(ansi: false)))->because('fast runs render nothing')->toBe('');
    }
    ...
    public function rendersSlowestFirstAndCapsAtFive(): void
    {
        ...
        expect($lines[0])->because('renders slowest first and caps at five')->toBe('Slowest tests:');
        expect(\count($lines))->toBe(6);
        expect($lines[1])->toBe('  0.580s Acme\SlowTest::case08');
        expect($lines[5])->toBe('  0.540s Acme\SlowTest::case04');
        expect($rendered)->not()->toContain('case03');
    }
    ...
    public function extendedModeKeepsMoreEntries(): void
    {
        ...
        expect($slow->render(new Style(ansi: false)))->because('extended mode keeps more entries')->toContain('case01');
    }
    ...
    public function durationsAreColoredThroughTheStyle(): void
    {
        ...
        expect($slow->render(new Style(ansi: true)))->because('durations are colored through the style')->toContain("\x1b[33m1.500s\x1b[0m");
    }
