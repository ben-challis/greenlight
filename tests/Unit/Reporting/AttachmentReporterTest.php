...
final class AttachmentReporterTest
{
    ...
    public function humanReportersListMetadataWithoutInliningContent(): void
    {
        foreach ([
            ...
            expect($output->buffer())->toContain('response.json')
    ...
    public function ttyReporterDoesNotRetainSuccessfulResultsUntilFinish(): void
    {
        ...
        expect($reference->get())->because('TTY reporter does not retain successful results until finish')->toBeNull();

        $reporter->finish();

        expect($output->buffer())->because('TTY reporter does not retain successful results until finish')->toContain('Retained attachments from successful tests:')
            ->toContain('response.json');
    }
    ...
    public function machineReportersExposePathsWithoutEmbeddingStorageState(): void
    {
        ...
        expect($json->buffer())->because('machine reporters expose paths without embedding storage state')->toContain('"path":"build/greenlight-artifacts/run-1/response.json"')
        ...
        expect($xml->buffer())->because('machine reporters expose paths without embedding storage state')->toContain('[[ATTACHMENT|build/greenlight-artifacts/run-1/response.json]]');
    }
    ...
    public function ciReportersLinkAndPublishTheArtifactDirectory(): void
    {
        ...
        expect($githubOutput->buffer())->because('ci reporters link and publish the artifact directory')->toContain('response.json')
        ...
        expect($teamCityOutput->buffer())->because('ci reporters link and publish the artifact directory')->toContain("##teamcity[testMetadata")
