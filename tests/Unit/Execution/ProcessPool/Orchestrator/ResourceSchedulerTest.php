...
final class ResourceSchedulerTest
{
    ...
    public function sharedResourceCapacityReducesTheInitialWorkerTarget(): void
    {
        ...
        expect($scheduler->initialWorkerTarget(8))
    ...
    public function initialWorkerTargetKeepsAllAchievableParallelism(): void
    {
        ...
        expect($scheduler->initialWorkerTarget(8))
            ...
            ->toBe(4);
        expect($scheduler->initialWorkerTarget(2))
    ...
    public function unconfiguredResourcesAreExclusive(): void
    {
        ...
        expect($scheduler->dispatch(true)->kind)->because('unconfigured resources are exclusive')->toBe(DispatchKind::Wait);

        $scheduler->release($first);

        expect(SchedulingFixture::assignedLease($scheduler)->unit->plan->classes())->because('unconfigured resources are exclusive')->toBe(['SecondTest']);
    }
    ...
    public function configuredLimitsPermitThatManyConcurrentLeases(): void
    {
        ...
        expect($scheduler->dispatch(true)->kind)->because('configured limits permit that many concurrent leases')->toBe(DispatchKind::Wait);
    }
    ...
    public function oldestBlockedUnitReservesItsResourcesButDisjointWorkCanBypassIt(): void
    {
        ...
        expect($disjoint->unit->plan->classes())->because('oldest blocked unit reserves its resources but disjoint work can bypass it')->toBe(['DisjointTest']);
        expect($scheduler->dispatch(true)->kind)->because('oldest blocked unit reserves its resources but disjoint work can bypass it')->toBe(DispatchKind::Wait);

        $scheduler->release($holdingB);

        expect(SchedulingFixture::assignedLease($scheduler)->unit->plan->classes())->because('oldest blocked unit reserves its resources but disjoint work can bypass it')->toBe(['NeedsBothTest']);
    }
    ...
    public function workCanUseCapacityBeyondTheOldestBlockedUnitReservation(): void
    {
        ...
        expect(SchedulingFixture::assignedLease($scheduler)->unit->plan->classes())->because('work can use capacity beyond the oldest blocked unit reservation')->toBe(['FirstNeedsATest']);
        expect($scheduler->dispatch(true)->kind)->because('work can use capacity beyond the oldest blocked unit reservation')->toBe(DispatchKind::Wait);
    }
    ...
    public function isolatedUnitsNeedAFreshWorkerAndRunAfterThePooledQueue(): void
    {
        ...
        expect($scheduler->dispatch(false)->kind)->because('isolated units need a fresh worker and run after the pooled queue')->toBe(DispatchKind::Drain);
        expect(SchedulingFixture::assignedLease($scheduler, freshWorker: true)->unit)->because('isolated units need a fresh worker and run after the pooled queue')->toBe($isolated);
    }
    ...
    public function requeuedIsolatedUnitsStillNeedAFreshWorker(): void
    {
        ...
        expect($scheduler->dispatch(false)->kind)->because('requeued isolated units still need a fresh worker')->toBe(DispatchKind::Drain);
        expect(SchedulingFixture::assignedLease($scheduler, freshWorker: true)->unit)->because('requeued isolated units still need a fresh worker')->toBe($isolated);
    }
