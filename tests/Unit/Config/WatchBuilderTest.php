...
final class WatchBuilderTest
{
    ...
    public function buildsTheDefaultAndConfiguredDebounce(): void
    {
        ...
        expect($default->debounceMilliseconds)
            ->because('watch mode MUST have a stable default debounce')
            ->toBe(200);

        expect($configured->debounceMilliseconds)
    ...
    public function accumulatesWatchInputsPatternsAndAFileLimit(): void
    {
        ...
        expect($configuration->paths)->toBe(['templates', 'config/app.yaml', 'migrations']);
        expect($configuration->includePatterns)->toBe(['**/*.twig', '**/*.yaml', '**/*.sql']);
        expect($configuration->excludePatterns)->toBe(['build/**', 'var/greenlight/**']);
        expect($configuration->maximumFiles)->toBe(500);
    }
