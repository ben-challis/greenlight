...
final class PhpStanNativeMatcherOverrideTest
{
    ...
    public function runtimeAndToolingRejectNativeMatcherNames(): void
    {
        ...
        Expect::value($run->exitCode)->not()->toBe(0);
        Expect::value($run->output())->toContain(self::MESSAGE);

        $helper = GreenlightCli::run($root, ['ide-helper', '--config=' . $config, '--no-ansi']);
        Expect::value($helper->exitCode)->not()->toBe(0);
        Expect::value($helper->output())->toContain(self::MESSAGE);
        ...
        Expect::value($analysis->exitCode)->not()->toBe(0);
        Expect::value($analysis->output())->toContain('conflicts with a native expectation method. Rename the extension matcher.');
    }
}
