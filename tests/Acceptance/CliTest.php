...
final readonly class CliTest
{
    ...
    public function printsTheResolvedPlanForAFixtureConfig(): void
    {
        ...
        Expect::value($result->exitCode)->because('prints the resolved plan for a fixture configuration')->toBe(0);
        Expect::value($output)->because('prints the resolved plan for a fixture configuration')
    ...
    public function commandLineFlagsOverrideTheConfigFile(): void
    {
        ...
        Expect::value($result->exitCode)->because('command line flags override the configuration file')->toBe(0);
        Expect::value($output)->because('command line flags override the configuration file')
            ->toContain('  workers: 2')
            ->toContain('  stop after: 7 failed or errored tests')
            ->toContain('  order: random (seed 9)')
            ->toContain('  groups: slow');
        Expect::value($output)->because('command line flags override the configuration file')->toContain('  resource limits: postgres=2');
    }
    ...
    public function helpAndVersionExitZero(): void
    {
        ...
        Expect::value($result->exitCode)->because('help and version exit zero')->toBe(0);
        ...
        Expect::value($result->exitCode)->because('help and version exit zero')->toBe(0);
        Expect::value($result->outputLines())->toContain('Greenlight ' . Application::VERSION);
    }
    ...
    public function runExecutesAPassingSuiteAndExitsZero(): void
    {
        ...
        Expect::value($result->exitCode)->because('run executes a passing suite and exits zero')->toBe(0);
        Expect::value($result->output())->because('run executes a passing suite and exits zero')->toContain('1 test, 1 passed');
        Expect::value($result->output())->because('run executes a passing suite and exits zero')->not()->toContain('alpha:one');
    }
    ...
    public function noAnsiAndVerboseAreAcceptedAndOutputStaysEscapeFree(): void
    {
        ...
        Expect::value($result->exitCode)->because('no ANSI and verbose are accepted and output stays escape free')->toBe(0);
        Expect::value($result->output())->because('no ANSI and verbose are accepted and output stays escape free')->not()->toContain("\x1b[");
        Expect::value($result->output())->because('no ANSI and verbose are accepted and output stays escape free')->toContain('1 test, 1 passed');
    }
    ...
    public function runExecutesAFailingSuiteAndExitsOne(): void
    {
        ...
        Expect::value($result->exitCode)->because('run executes a failing suite and exits one')->toBe(1);
        Expect::value($result->output())->because('run executes a failing suite and exits one')->toContain('intentional boom');
    }
    ...
    public function runWithNoTestsExitsOne(): void
    {
        ...
        Expect::value($result->exitCode)->because('run with no tests exits one')->toBe(1);
        Expect::value($result->output())->because('run with no tests exits one')->toContain('Greenlight found no tests');
    }
    ...
    public function listTestsPrintsDiscoveredTestIds(): void
    {
        ...
        $output = $result->outputLines();
        Expect::value($result->exitCode)->because('list tests prints discovered test IDs')->toBe(0);
        Expect::value($output)->because('list tests prints discovered test IDs')
    ...
    public function listTestsHonorsGroupFilters(): void
    {
        ...
        $output = $result->outputLines();
        Expect::value($result->exitCode)->because('list tests honors group filters')->toBe(0);
        Expect::value($output)->because('list tests honors group filters')
            ->toContain('Greenlight\Tests\Fixture\DiscoveryBasic\AlphaTest::two')
    ...
    public function missingConfigFileFailsWithAnActionableMessage(): void
    {
        ...
        Expect::value($result->exitCode)->because('missing configuration file fails with an actionable message')->toBe(1);
        Expect::value($result->output())->because('missing configuration file fails with an actionable message')->toContain('greenlight: No greenlight.php found in');
    }
    ...
    public function unknownOptionsAreUsageErrors(): void
    {
        ...
        Expect::value($result->exitCode)->because('unknown options are usage errors')->toBe(64);
        Expect::value($result->output())->because('unknown options are usage errors')->toContain('greenlight: Unknown option "--frobnicate"');
        Expect::value($result->output())->because('unknown options are usage errors')->not()->toContain("\x1b[");
    }
