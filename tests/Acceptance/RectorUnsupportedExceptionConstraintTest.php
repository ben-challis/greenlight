...
final readonly class RectorUnsupportedExceptionConstraintTest
{
    ...
    public function leavesUnsupportedExceptionConstraintsUntouched(): void
    {
        ...
        foreach ($probes as $caseName => $probe) {
            Expect::value($probe->changed)->because('unsupported exception constraint: ' . $caseName)->toBeFalse();
            Expect::value($probe->code)->because('unsupported exception constraint: ' . $caseName)->toBe($cases[$caseName]);
        }
    }
}
