...
final readonly class CommandErrorsTest
{
    ...
    public function unknownCommandExitsWithAUsageError(): void
    {
        ...
        expect($result->exitCode)->because('unknown command exits with a usage error')->toBe(64);
        expect($result->output())->toContain("Unknown command 'bogus-command'")
    ...
    public function coverageDiffWithoutBaselineOrCurrentIsAUsageError(): void
    {
        ...
        expect($result->exitCode)->because('coverage diff without baseline or current is a usage error')->toBe(64);
        expect($result->output())->toContain('coverage:diff requires --baseline=<path> and --current=<path>');
    }
    ...
    public function profileReportWithAMissingInputFileFailsCleanly(): void
    {
        ...
        expect($result->exitCode)->because('profile report with a missing input file fails cleanly')->toBe(1);
        expect($result->output())->toContain('Greenlight could not read')
    ...
    public function ideHelperWithAnUnwritableOutputPathFailsCleanly(): void
    {
        ...
        try {
            ...
            expect($result->output())->toContain(\sprintf('Greenlight could not write "%s":', $outputPath));
        } finally {
