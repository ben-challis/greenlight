...
final readonly class PhpStanDataProviderShapeRuleTest
{
    ...
    public function providerAndRowShapesAreCheckedAgainstTheSignature(): void
    {
        ...
        Expect::value($probe->exitCode)->because('provider and row shapes are checked against the signature')->toBe(1);
        Expect::value($probe->goodPassed)->toBeTrue();
        Expect::value(\count($probe->errors))->toBe(11);
        Expect::value($probe->messages())->toContain('Data provider doesNotExist() for missingProvider() does not exist')
