...
final readonly class RectorUnsupportedArgumentShapeTest
{
    ...
    public function leavesUnsupportedArgumentShapesUntouched(): void
    {
        ...
        foreach ($probes as $caseName => $probe) {
            expect($probe->changed)->because('unsupported argument shape: ' . $caseName)->toBeFalse();
            expect($probe->code)->because('unsupported argument shape: ' . $caseName)->toBe($cases[$caseName]);
        }
    }
}
