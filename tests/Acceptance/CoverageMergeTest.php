...
final readonly class CoverageMergeTest
{
    ...
    public function mergesLineSetsAndWritesEachDeterministicExport(): void
    {
        ...
        Expect::value(CoverageJson::read($directory . '/out/coverage.json')->toWire())
            ->toBe([
                'files' => [
        ...
        Expect::value((string) \file_get_contents($directory . '/out/lcov.info'))
            ->toContain("SF:/project/src/A.php\n")
            ->toContain("DA:3,0\n")
            ->toContain("SF:/project/src/B.php\n");
        Expect::value((string) \file_get_contents($directory . '/out/clover.xml'))
            ->toContain('<file name="/project/src/A.php">');
        Expect::value((string) \file_get_contents($directory . '/out/cobertura.xml'))
            ->toContain('filename="project/src/A.php"');
        Expect::value((string) \file_get_contents($directory . '/out/html/index.html'))
            ->toContain('src/A.php')
            ->toContain('src/B.php');
        ...
        Expect::value($reverse->exitCode)->toBe(0);
        Expect::value((string) \file_get_contents($directory . '/reverse.json'))
    ...
    public function explicitRootsCreateOnePortableCoverageMap(): void
    {
        ...
        Expect::value($result->exitCode)
            ...
            ->toBe(0);
        Expect::value(CoverageJson::read($directory . '/merged.json')->toWire())
            ->toBe([
                'files' => [
    ...
    public function mergesEmptyMapsIntoOneDeterministicEmptyDocument(): void
    {
        ...
        Expect::value($result->exitCode)
            ...
            ->toBe(0);
        Expect::value((string) \file_get_contents($directory . '/merged.json'))
    ...
    public function appliesCoverageGatesToTheMergedMapAfterItWritesExports(): void
    {
        ...
        Expect::value(\is_file($directory . '/merged.json'))
