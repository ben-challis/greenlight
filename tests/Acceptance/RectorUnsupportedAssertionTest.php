...
final readonly class RectorUnsupportedAssertionTest
{
    ...
    public function leavesClassesWithUnsupportedAssertionsUntouched(): void
    {
        ...
        foreach ($probes as $caseName => $probe) {
            expect($probe->changed)->because('unsupported assertion case: ' . $caseName)->toBeFalse();
            expect($probe->code)->because('unsupported assertion case: ' . $caseName)->toBe($cases[$caseName]);
        }
    }
}
